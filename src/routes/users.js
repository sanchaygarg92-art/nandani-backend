...
router.get('/me', verifyToken, async (req, res) => {
  try {
    ...
    const addrs = await query('SELECT * FROM addresses WHERE user_uid = $1 ORDER BY is_default DESC, created_at DESC', [req.uid]);
...
router.post('/me/addresses', verifyToken, async (req, res) => {
  ...
  try {
    ...
    if (exists.rows.length > 0) return res.json({ id: exists.rows[0].id, address: address.trim() });
...
router.get('/all', verifyAdmin, async (req, res) => {
  try {
    ...
    const result = await Promise.all(users.rows.map(async (user) => {
      const addrs = await query('SELECT address, label FROM addresses WHERE user_uid = $1', [user.uid]);
